...
namespace byteurl::postgres {
    ...
    drogon::Task<std::optional<LinkStats>> LinkRepository::getStats(const std::string &shortCode) try {
        auto result = co_await dbClient_->execSqlCoro(
                "SELECT short_code, original_url, hits, created_at FROM links WHERE short_code = $1",
                shortCode
        );
        ...
        LinkStats stats;
        stats.shortCode   = result[0]["short_code"].as<std::string>();
        stats.originalUrl = result[0]["original_url"].as<std::string>();
        stats.hits        = result[0]["hits"].as<long long>();
        stats.createdAt   = result[0]["created_at"].as<std::string>();
        co_return stats;
    } catch (const std::exception &e) {
        LOG_ERROR << "Postgres error in LinkRepository::getStats: " << e.what();
        co_return std::nullopt;
    } catch (...) {
        LOG_ERROR << "Unknown error in LinkRepository::getStats";
        co_return std::nullopt;
    }
