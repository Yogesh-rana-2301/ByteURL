...
namespace byteurl::redis {
    ...
    // Stats are not stored in Redis — always return nullopt so the service
    // falls back to the Postgres repository for authoritative data.
    drogon::Task<std::optional<LinkStats>> LinkRepository::getStats(const std::string &) {
        co_return std::nullopt;
    }
